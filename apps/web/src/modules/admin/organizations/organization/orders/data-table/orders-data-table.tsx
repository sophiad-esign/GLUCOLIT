...
import { useColumns } from "~/modules/admin/users/user/orders/data-table/columns";
...
export const OrdersDataTable = ({ organizationId }: OrdersDataTableProps) => {
  ...
  const { table, query } = useDataTable({
    ...
    query: ({ page, perPage, sorting, filters }) =>
      admin.queries.organizations.getOrders({
        id: organizationId,
        page: page.toString(),
