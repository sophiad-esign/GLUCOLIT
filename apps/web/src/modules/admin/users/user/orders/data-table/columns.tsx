...
import type { Order } from "@workspace/billing";
...
export const useColumns = (): ColumnDef<Order>[] => {
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const statusKey = `order.status.${row.original.status.replace(/_([a-z])/g, (_, letter: string) => letter.toUpperCase())}`;
      ...
      meta: {
        ...
        options: Object.values(PaymentStatus).map((status) => {
          const statusKey = `order.status.${status.replace(/_([a-z])/g, (_, letter: string) => letter.toUpperCase())}`;
          return {
